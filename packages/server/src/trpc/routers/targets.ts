...
import { z } from 'zod'
import { db, targets, targetCurrentStatus } from '../../db'
...
export const targetsRouter = router({
	list: protectedProcedure.query(async (): Promise<TargetWithStatus[]> => {
		const result = await db
			.select()
			.from(targets)
			.leftJoin(targetCurrentStatus, eq(targets.id, targetCurrentStatus.targetId))
	...
	get: protectedProcedure.input(z.object({ id: z.string().uuid() })).query(async ({ input }) => {
		...
		return {
			...
			lastResponseTimeMs: row.target_current_status?.lastResponseTimeMs ?? null,
		} as TargetWithStatus
	}),

	create: protectedProcedure.input(createTargetInputSchema).mutation(async ({ input }) => {
	...
	update: protectedProcedure.input(updateTargetInputSchema).mutation(async ({ input }) => {
