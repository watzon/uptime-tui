import { metricsQueryInputSchema, uptimeSummaryInputSchema, type Metric, type UptimeSummary } from '@downtime/shared'
import { and, avg, count, eq, gte, lte, sql } from 'drizzle-orm'
import { db, metrics } from '../../db'
import { protectedProcedure, router } from '../trpc'

export const metricsRouter = router({
	query: protectedProcedure.input(metricsQueryInputSchema).query(async ({ input }): Promise<Metric[]> => {
		...
		if (aggregation === 'raw') {
			const result = await db
				.select()
				.from(metrics)
				.where(and(eq(metrics.targetId, targetId), gte(metrics.time, startTime), lte(metrics.time, endTime)))
		...
		const bucketSeconds =
			aggregation === '1m' ? 60 : aggregation === '5m' ? 300 : aggregation === '1h' ? 3600 : 86400
	...
	summary: protectedProcedure.input(uptimeSummaryInputSchema).query(async ({ input }): Promise<UptimeSummary> => {
		...
		const failedChecks = parseInt(row.failed_checks, 10)
		const avgResponseTimeMs = row.avg_response_time_ms ? Math.round(parseFloat(row.avg_response_time_ms)) : null
