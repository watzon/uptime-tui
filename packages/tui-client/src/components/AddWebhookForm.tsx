...
export function AddWebhookForm() {
	...
	const [events, setEvents] = useState<string[]>(['up', 'down', 'error'])
	...
	const handleSubmit = async () => {
		...
		try {
			...
			setErrors({
				submit: err instanceof Error ? err.message : 'Failed to create webhook',
			})
	...
	const footer = (
		<Text dimColor>
			...
			{focusedField === 'enabled' ? 'submit' : 'next'} | Esc: cancel
		</Text>
	)
